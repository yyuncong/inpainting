...
def main():
    ...
    image_preprocessor = CLIPImageProcessor.from_pretrained(
    ...
    def collate_fn(examples):
        ...
        processed_prompt = image_preprocessor(
            input_timestep_pixel_values.squeeze(1).to(weight_dtype) / 2 + 0.5,
            do_rescale=False,
            return_tensors="pt",
        )["pixel_values"]
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                encoder_hidden_states = image_encoder(
                    **{"pixel_values": batch["prompt_pixel_values"].to(weight_dtype)}
                ).last_hidden_state
                ...
                if args.conditioning_dropout_prob is not None:
                    ...
                    null_conditioning = {
                        "pixel_values": torch.zeros_like(
                            batch["prompt_pixel_values"].to(weight_dtype),
                            device=latents.device,
                        ),
                    }
                    null_conditioning = image_encoder(
                        **null_conditioning
                    ).last_hidden_state
